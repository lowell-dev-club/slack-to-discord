...
class Music:
    ...
    async def skip(self, ctx):
        ...
        if ctx.channel.permissions_for(ctx.author).administrator:
            ...
            needed_votes = int(len(state.skip_votes) / required_votes)
            await ctx.send(f"{ctx.author.mention} voted to skip ({needed_votes} votes)")
        ...
        if user != self.bot.user and message.author == self.bot.user:
            ...
            if message.guild and message.guild.voice_client:
                ...
                if permissions.administrator or (user_in_channel and state.is_requester(user)):
                    ...
                    needed_votes = int(len(state.skip_votes) / required_votes)
                    await channel.send(
                        f"{user.mention} voted to skip ({needed_votes} votes)"
                    )
